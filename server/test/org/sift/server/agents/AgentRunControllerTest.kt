...
class AgentRunControllerTest {
    ...
    fun `POST creates a run and returns 202 with location`() {
        ...
        every { service.create(expected) } returns run
    ...
    fun `domain exceptions map to 404 409 and 500 problem details`() {
        ...
        mockMvc.get("/api/v1/agents/$id").andExpect {
            status { isNotFound() }
            jsonPath("$.detail") { value("Agent run $id not found") }
        }

        every { service.cancel(id) } throws ConflictException("already SUCCESS")
        mockMvc.post("/api/v1/agents/$id/cancel").andExpect {
            status { isConflict() }
            jsonPath("$.detail") { value("already SUCCESS") }
        }

        every { service.create(any()) } throws KubernetesClientException("forbidden")
        mockMvc.post("/api/v1/agents") {
            contentType = MediaType.APPLICATION_JSON
            content = body
        }.andExpect {
    ...
    fun `GET list passes filters and paging and GET by id returns the run`() {
        ...
        mockMvc.get("/api/v1/agents?$query").andExpect {
            status { isOk() }
        ...
        mockMvc.get("/api/v1/agents?phase=EXPLODED").andExpect { status { isBadRequest() } }
        mockMvc.get("/api/v1/agents/$id").andExpect {
            status { isOk() }
        ...
        mockMvc.get("/api/v1/agents/not-a-uuid").andExpect { status { isNotFound() } }
    }
